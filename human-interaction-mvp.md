# Human Interaction MVP for Asahi LUKS Encryption
...
Assumption: an AI agent such as Codex is already running in macOS. The AI handles the macOS-side work: downloading tools, checking sources, preparing QEMU, discovering partitions, running preflight, starting the helper VM, and cleaning up only after authorization.
...
3. Keep holding until the startup options screen appears.
4. Choose the Fedora/Asahi boot option.
5. Do not choose macOS for this first encrypted boot.
